...
class ChickenMasterV4:
    ...
    def full_auto_loop(self):
        while self.auto_running:
            # [수정 1] 새로운 사이클 시작 전 타겟 데이터 초기화
            self.current_target = None 
            
            # [수정 2] 안전 위치로 이동 시 '확실히' 도착할 때까지 대기
            self.add_log("🚚 안전 위치(스캔 지점)로 이동 중...")
            # send_command_and_wait 내부에서 버퍼를 비우지만, 
            # 루프 사이클이 너무 빠를 경우를 대비해 MOVE 명령을 확실히 체크합니다.
            move_success = self.send_command_and_wait(f"MOVE,{SAFE_APPROACH[0]},{SAFE_APPROACH[1]},{SAFE_APPROACH[2]},0,180,0")
            
            if not move_success:
                self.add_log("❌ 안전 위치 이동 실패. 루프를 중단합니다.")
                break
            
            # [수정 핵심] 로봇이 멈춘 후 카메라 영상이 흔들리지 않고 최신 프레임을 가져오도록 대기
            time.sleep(1.5) 

            # 2. 감지 실행 (이제 안전 위치에서 정지된 상태의 영상을 분석합니다)
            self.scan_chicken()
            
            if self.current_target is None:
                self.add_log("💤 물체 없음... 재시도")
                # 홈으로 가지 않고 안전 위치에서 잠시 대기 후 재스캔
                time.sleep(1.0)
                continue
