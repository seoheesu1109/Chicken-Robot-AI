...
SCOOP_EAST = [
    ...
    {'x': 764.00, 'y': 856.00, 'z': 860.00, 'rx':   6.35, 'ry': 130.00, 'rz':-172.00},
    {'x': 764.00, 'y': 856.00, 'z': 860.00, 'rx':  86.35, 'ry':-130.00, 'rz':-0.1},
    {'x': 565.38, 'y': 66.23, 'z': 740.82, 'rx': 84.21,  'ry':-128.83, 'rz':  -0.91},
    {'x': 503.11, 'y': -422.21, 'z': 944.55, 'rx': 91.21,  'ry':150.83, 'rz':  3.81},
    {'x': 637.27, 'y': -562.09, 'z': 911.77, 'rx': 93.71,  'ry':126.48, 'rz':  0.14},
    {'x': 565.38, 'y': 66.23, 'z': 740.82, 'rx': 84.21,  'ry':-128.83, 'rz':  -0.91},
...
SCOOP_NORTH = [
    ...
    {'x': 766.46, 'y': 914.60, 'z': 797.00, 'rx':  86.48, 'ry': 123.00, 'rz':177.00},
    ...
    def _move_worker(self, mode, target_pos_at_click):
        ...
        # [추가] RZ 자동 계산 이동 버튼용
            ...
            if bx > 900:
                ...
                selected = SCOOP_NORTH
                direction = "북"
            else:
                selected = SCOOP_SOUTH
                direction = "남"

            self.status_label.configure(text=f"🥄 Scooping {direction}...", text_color="#1abc9c")
            print(f"🥄 방향: {direction} | Pixel X: {bx:.1f}")
