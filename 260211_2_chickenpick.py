...
#HOME_ORI = [0.0, 180.0, 0.0]
#HOME_POS = [424.17, -69.51, 742.91] + HOME_ORI
...
class ChickenMasterV4:
    ...
    def scan_chicken(self):
        ...
        if best_target:
            ...
            if best_target:
                u, v = best_target['u'], best_target['v']
                self.target_pixel = (u, v)
    ...
    def vision_worker(self):
        """실시간 화면 출력: 모든 후보(노랑)와 최종 타겟(초록+빨간점) 표시"""
        ...
        try:
            while True:
                ...
                # 현재 scan_chicken에서 계산된 타겟 좌표 가져오기
                target = self.current_target # [final_x, final_y, final_z, rx, ry, angle]

                # 1. 모든 후보군 표시 (얇은 노란색)
                for c in cnts:
                    if cv2.contourArea(c) > 400:
                        cv2.drawContours(disp, [c + (x1, y1)], -1, (0, 255, 255), 1)

                # 2. 최종 선택된 타겟 강조 (초록색 테두리 + 빨간 점)
                if target:
                    # 로봇 좌표(final_x, final_y)를 다시 이미지 좌표(u, v)로 역계산하는 대신
                    # scan_chicken에서 저장한 u, v를 사용하는 것이 정확하지만, 
                    # 여기서는 현재 타겟 위치를 화면에 찍어주기 위해 간단한 매핑을 시각적으로 구현합니다.
                    
                    # 로봇 좌표계 보정을 거치기 전의 픽셀 위치를 찾기 위해
                    # scan_chicken에서 'last_u', 'last_v'를 저장해두면 더 정확합니다.
                    # (일단은 scan_chicken이 실행될 때 결정된 좌표를 표시하도록 current_target 기반으로 추정)
                    
                    # 만약 scan_chicken에서 self.target_pixel = (u, v)를 저장한다면:
                    if hasattr(self, 'target_pixel'):
                        tu, tv = self.target_pixel
                        # 타겟 테두리 (굵은 초록색)
                        cv2.circle(disp, (tu, tv), 12, (0, 255, 0), 2)
                        # 타겟 중심점 (빨간 점)
                        cv2.circle(disp, (tu, tv), 5, (0, 0, 255), -1)
                        cv2.putText(disp, "PICK TARGET", (tu - 40, tv - 20), 
