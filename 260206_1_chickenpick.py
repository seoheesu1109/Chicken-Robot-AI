...
class ChickenMasterV4:
    ...
    def scan_chicken(self):
        ...
        if valid_targets:
            # [핵심 변경] v값(Y좌표)이 가장 작은 것(=화면에서 가장 위쪽)을 선택
            ...
            if dist > 0:
                ...
                real_z = dst[0][0][2] + Z_FINE_TUNE
                self.current_target = [dst[0][0][0], dst[0][0][1] + Y_OFFSET, real_z, FIXED_ORI[0], FIXED_ORI[1], angle]
                self.add_log(f"🎯 최상단 타겟 고정: Y_pixel={v}, Z={real_z:.1f}")
            else:
    ...
    def go_home(self): self.send_command(f"MOVE,{HOME_POS[0]},{HOME_POS[1]},{HOME_POS[2]},0,180,0")
